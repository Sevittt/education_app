...
class _AdminAddEditNewsScreenState extends State<AdminAddEditNewsScreen> {
  ...
  void initState() {
    ...
    _selectedPublicationDate =
        widget.newsItemToEdit?.publicationDate?.toDate() ?? DateTime.now();
  }
  ...
  Future<void> _saveNews() async {
    ...
    final newsData = News(
      ...
      publicationDate: Timestamp.fromDate(_selectedPublicationDate!),
      // You could add 'createdBy' or 'lastEditedBy' fields here using adminUserId
    );

    try {
      if (_isEditing) {
        await newsService.updateNews(widget.newsItemToEdit!.id, newsData);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
