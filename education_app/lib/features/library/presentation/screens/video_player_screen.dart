...
class _VideoPlayerScreenState extends State<VideoPlayerScreen> {
  ...
  void initState() {
    ...
    // DEBUG: Check platform and video data
    debugPrint('🎬 Video Player Debug Info:');
    debugPrint('  Platform: ${kIsWeb ? "Web" : "Mobile"}');
    debugPrint('  YouTube ID: "$_youtubeId"');
    debugPrint('  Video Title: "$_videoTitle"');
    debugPrint('  Thumbnail: "$_thumbnailUrl"');
    debugPrint('  Video Entity ID: "$_videoId"');

    // Increment views via provider
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted)
        context.read<LibraryProvider>().incrementVideoViews(_videoId);
    });
    ...
    debugPrint('✅ YouTube Player Controller initialized');
  }
  ...
  Future<void> _awardPointsForWatching() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
                Text(l10n.pointsEarned(GamificationRules.xpVideoComplete))),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_youtubeId.isEmpty) {
      debugPrint('❌ Error: YouTube ID is empty!');
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                'YouTube ID mavjud emas',
                style: Theme.of(context).textTheme.titleMedium,
                textAlign: TextAlign.center,
              ),
              const SizedBox(height: 8),
              Text(
                'Video ma\'lumotlari to\'liq emas',
