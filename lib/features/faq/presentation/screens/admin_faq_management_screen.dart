...
class _AdminFAQManagementScreenState extends State<AdminFAQManagementScreen> {
  // Service removed, using Notifier via Provider

  Future<void> _deleteFAQ(FaqEntity faq) async {
    ...
    if (confirmed == true && mounted) {
      try {
        await notifier.deleteFaq(faq.id);
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<FaqNotifier>(
        builder: (context, notifier, child) {
          return StreamBuilder<List<FaqEntity>>(
            stream: notifier.faqsStream,
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Center(
                    child:
                        SelectableText('${l10n.errorPrefix}${snapshot.error}'));
              }
