...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _updateNotificationSetting(bool newValue) async {
    ...
    if (mounted) {
      ...
      if (authNotifier.appUser != null) {
        final updatedUser =
            authNotifier.appUser!.copyWith(notificationsEnabled: newValue);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            newValue ? "Xabarnomalar yoqildi" : "Xabarnomalar o'chirildi",
          ),
        ),
      );
    }
        ...
        if (!await launchUrl(url)) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content:
                      Text(AppLocalizations.of(context)!.errorTelegramOpen)),
            );
          }
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    final currentLanguage =
        _getLanguageName(localeProvider.appLocale?.languageCode ?? 'en');
      ...
      body: ListView(
        children: [
          ...
          _buildSwitchSettingsItem(
            ...
            subtitle: l10n?.settingsReceiveNotificationsSubtitle ??
          ...
          //   subtitle: l10n?.settingsAllowLocationSubtitle ??
          ...
          _buildSettingsItem(
            ...
            onTap: () {
              showAboutDialog(
                context: context,
                applicationName: l10n?.appTitle ?? 'Court handbook',
                ...
                applicationLegalese: l10n?.settingsAppLegalese ??
          ...
          // Only show for system admins (CourtRole.admin)
          if (context.watch<AuthNotifier>().appUser?.isAdmin == true)
            Column(
              children: [
                _buildSettingsItem(
                  context: context,
