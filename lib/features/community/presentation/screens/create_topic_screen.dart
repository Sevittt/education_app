    ...
    try {
      ...
      if (widget.topic != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(l10n?.topicUpdatedSuccessfully ??
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${l10n?.errorGeneric ?? 'Error'}: $e')),
        );
      }
    } finally {
      if (mounted) {
    ...
    return Scaffold(
      appBar: AppBar(
          title: Text(isEditing
              ? (l10n?.editTopic ?? 'Edit Topic')
              : (l10n?.createTopicScreenTitle ?? 'Start Discussion'))),
      body: Padding(
              ...
              _isLoading
                  ...
                  : ElevatedButton(
                      ...
                      child: Text(isEditing
                          ? (l10n?.update ?? 'Update')
                          : (l10n?.createTopicButtonText ?? 'Create')),
                    ),
            ],
          ),
        ),
