...
class AdminAppUserListScreen extends StatelessWidget {
  ...
  Color _getRoleColor(CourtRole role, ColorScheme colorScheme) {
    switch (role) {
      case CourtRole.admin:
        return colorScheme.error; // Admin — qizil/muhim
      case CourtRole.judge:
      case CourtRole.ict_specialist:
  ...
  Future<void> _showRoleChangeDialog(
    ...
    final result = await showDialog<CourtRole>(
      context: context,
      builder: (ctx) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: CourtRole.values
                .where((r) => r != CourtRole.unknown)
                .map((role) {
              ...
              return ListTile(
                leading: Icon(
                  ...
                  color:
                      isCurrentRole ? Theme.of(ctx).colorScheme.primary : null,
                ),
    ...
    if (result != null && result != user.role && context.mounted) {
      try {
        await authRepository.updateCourtRole(user.id, result);
        if (context.mounted) {
