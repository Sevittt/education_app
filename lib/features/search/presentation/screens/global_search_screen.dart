...
class _GlobalSearchScreenState extends State<GlobalSearchScreen> {
      ...
      child: SizedBox(
        ...
        child: ListView(
          ...
          children: [
            ...
            _buildChip(
              label: l10n.searchFilterSystems,
              icon: Icons.computer,
            ...
            const SizedBox(width: 8),
            _buildChip(
              label: l10n.searchFilterCourses,
              icon: Icons.school_rounded,
              isSelected: activeFilter == SearchResultType.course,
              count: counts[SearchResultType.course] ?? 0,
              color: Colors.tealAccent,
              onTap: () => notifier.setFilter(SearchResultType.course),
            ),
          ],
        ),
      ),
  ...
  Widget _buildBody(
      SearchNotifier notifier, ThemeData theme, AppLocalizations l10n) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              l10n.searchNoResultsForQuery(notifier.lastQuery),
    ...
    return ListView(
      ...
      children: [
        ...
        if (directResults.isNotEmpty) ...[
          _buildSectionHeader(l10n.searchResultsHeader(directResults.length)),
          const SizedBox(height: 8),
        ...
        if (semanticResults.isNotEmpty) ...[
          const SizedBox(height: 16),
          _buildSectionHeader(
              l10n.searchSemanticHeader(semanticResults.length)),
          const SizedBox(height: 8),
                      ...
                      children: [
                        ...
                        if (isSemantic) ...[
                          ...
                          Container(
                            ...
                            child: Text(
                              AppLocalizations.of(context)!.searchSemanticBadge,
                              style:
    ...
    switch (type) {
      case SearchResultType.system:
        return _TypeConfig(
          ...
          color: Colors.greenAccent,
          label: l10n.searchTypeSystem,
        );
      case SearchResultType.article:
        return _TypeConfig(
          icon: Icons.article_rounded,
          color: Colors.blueAccent,
          label: l10n.searchTypeArticle,
        );
      case SearchResultType.video:
        return _TypeConfig(
          icon: Icons.play_circle_fill_rounded,
          color: Colors.redAccent,
          label: l10n.searchTypeVideo,
        );
      case SearchResultType.faq:
        return _TypeConfig(
          icon: Icons.help_outline_rounded,
          color: Colors.orangeAccent,
          label: l10n.searchTypeFaq,
        );
      case SearchResultType.resource:
        return _TypeConfig(
          icon: Icons.link_rounded,
          color: Colors.purpleAccent,
          label: l10n.searchTypeResource,
        );
      case SearchResultType.course:
        return _TypeConfig(
          icon: Icons.school_rounded,
          color: Colors.tealAccent,
          label: l10n.searchTypeCourse,
        );
    }
  }
  ...
  void _navigateToDetail(SearchResultEntity result) async {
    switch (result.type) {
      case SearchResultType.article:
        final article =
            await context.read<LibraryProvider>().getArticleById(result.id);
      ...
      case SearchResultType.video:
        final video =
            await context.read<LibraryProvider>().getVideoById(result.id);
      ...
      case SearchResultType.system:
        final system =
            await context.read<SystemsNotifier>().getSystemById(result.id);
      ...
      case SearchResultType.resource:
        ...
        break;
      case SearchResultType.course:
        // Ensure CourseProvider has loaded courses before navigating
        if (mounted) {
          ...
          if (mounted) {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => CourseDetailScreen(courseId: result.id),
              ),
            );
          }
        }
