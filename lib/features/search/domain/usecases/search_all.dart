...
class SearchAll {
  ...
  Future<List<SearchResultEntity>> call(String query) async {
    ...
    try {
      ...
      final courses = futures[4] as List<CourseEntity>;
      ...
      // --- Score and collect Courses ---
      for (final course in courses) {
        if (!course.isPublished) continue;
        final score = _scoreCourse(course, lowerQuery, queryWords);
        if (score > 0) {
          results.add(SearchResultEntity(
            id: course.id,
            title: course.title,
  ...
  /// Score a course based on query match.
  double _scoreCourse(CourseEntity course, String query, List<String> words) {
    double score = 0;
    ...
    // Role matching
    for (final role in course.targetRole) {
      if (role.toLowerCase().contains(query)) score += 4;
      for (final word in words) {
        if (role.toLowerCase().contains(word)) score += 2;
      }
    }
