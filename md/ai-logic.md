...
Go to your Firebase Extensions dashboard in the Firebase console.

On the installed extension instance's card, click Manage.
...
Go to your Functions dashboard of the Firebase console.

In the Dashboard tab, you can view the functions from Firebase Extensions (alongside any other functions that you've deployed for your project).
...
Open your project's Cloud Scheduler page in the Google Cloud console.

In the Jobs list, you can view the Cloud Scheduler jobs from Firebase Extensions (alongside any other jobs you've created for your project).
...
In the Firebase console Extensions section, open the extension instance's details page.

On the details page, open the APIs and resources section. If the extension uses Cloud Tasks, there will be a Cloud Task queues section with one or more entries.

Click View queue for the queue you want to manage. This will open the Queue Details page in the Google Cloud console, from which you can view queued tasks, pause the queue, remove tasks from the queue, and so on. See Manage queues and tasks in the Cloud Tasks documentation.
...
Only after installing the new instance, uninstall the old instance.
View Cloud Secret Manager secrets created by extensions
Open your project's Secret Manager page in the Google Cloud console.

In the Secrets list, you can view the secrets created for Firebase Extensions (alongside any other secrets you've created for your project).
...
In the Health tab of your Functions dashboard, you can view an overview of errors and performance information for all functions in your project.

To view information for a specific extension, use the filter at the top of the page to select a specific function.

Check how often an installed extension is running
In the Dashboard tab of your Functions dashboard, locate the specific function for the Firebase Extension that you want to check.

Click more_vert (the overflow menu) on the far right-side of the entry, then select Detailed usage stats.

In the displayed Google Cloud console, you can drill down into various invocations of a function and even inspect its source code.
...
In the upper-right corner, click Update extension.

If the extension doesn't have an available new version, the details page won't have an update button.

Review what's new in the update and configure the extension (if needed).
...
Follow the on-screen instructions to reconfigure the parameter values for your extension.
...
At the bottom of the screen, click Uninstall extension.

Review what will be deleted, then click Uninstall extension to confirm the deletion.
...
If it doesn’t exist already, create a collection called knowledge_base.

Create a document with a field named text, and add text you would like to embed.

In a few seconds, you’ll see a new field called embedding pop up in the same document you just created. It will contain the embedding for the text in text
...
https://console.firebase.google.com/project/educationapp-4780a/firestore/indexes
...
Or by adding a document to the collection _firestore-vector-search/index/queries with a query field and an (optional) limit field.
...
This extension publishes events to the projects/educationapp-4780a/locations/us-central1/channels/firebase channel. You can create custom handlers that respond to events on this channel. For example:
